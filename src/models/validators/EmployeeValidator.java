...
public class EmployeeValidator { //バリデーション用クラス
...
private static String validateCode(String code,Boolean codeDuplicateCheckFlag){
    ...
    if(codeDuplicateCheckFlag){
        ...
        long employees_count=(long)em.createNamedQuery("checkRegisteredCode",Long.class)//指定された社員番号がすでにデータベースに存在しているかを調べて(EmployeeクラスのcheckRegisteredCode)でLong型で受け取る
                                      .setParameter("code",code)//検索する社員番号をcheckRegisteredCodeの"SELECT COUNT(e) FROM Employee AS e WHERE e.code = :code"の:codeにセットする

                                      .getSingleResult();//結果は一件のみ受け取る
        em.close();
